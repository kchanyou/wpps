...
class ServerFragment :
    ...
    private fun requestPermission() {
        RxPermission.getInstance(requireContext())
            ...
            .requestForFullResult { isGranted ->
                if (isGranted) {
                    initCamera()
                    // --- (변경) ---
                    // ViewModel의 startServer()를 호출하여 그룹 생성을 시작
    ...
    private fun bindGroupState(group: WifiP2pInfo?) {
        viewBinding().apply {
            ...
            if (group != null) {
                requireContext().showToast(R.string.group_created)
            } else {
                // 그룹이 제거되었을 때만 토스트 메시지를 표시하도록 수정 (초기 null 상태 제외)
                if (btnGroup.drawable.constantState != resources.getDrawable(R.drawable.ic_wifi_tether_off, null).constantState) {
                    requireContext().showToast(R.string.group_removed)
                }
            }
        }
    }
    ...
    // --- (제거) ---
    // initWifiDirect 함수는 더 이상 필요 없으므로 제거
